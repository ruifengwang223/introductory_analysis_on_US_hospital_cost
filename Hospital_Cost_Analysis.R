...
### Additional analysis: Add map
# Step 1: Mean year-to-year percent change
state_tcpc_rates <- state_tcpc %>%
  rowwise() %>%
  mutate(
    rate = mean( c(
      (`2018` - `2017`) / `2017`,
...
state_map_base <- state_tcpc_rates %>%
  mutate(region = tolower(state_name))

yearly_long <- state_map_base %>%
  pivot_longer(
    cols      = `2017`:`2022`,
    names_to  = "year",
    values_to = "tcpc"
...
yearly_map <- left_join(us_states, yearly_long, by = "region")

ggplot(yearly_map, aes(x=long, y=lat, group=group, fill=tcpc)) +
  geom_polygon(color="white", size=0.1) +
  facet_wrap(~ year) +
  scale_fill_gradientn(
    colours = c("white", "#a6cee3", "#1f78b4", "#6a3d9a", "#4b0082"),
    na.value = "grey90",
    name     = "Cost per Capita"
  ) +
  labs(
    title = "Hospital Cost per Capita by State (2017–2022)",
    x     = NULL,
    y     = NULL
  ) +
  theme_void() +
...
rate_map <- left_join(us_states, state_map_base, by = "region")

ggplot(rate_map, aes(x = long, y = lat, group = group, fill = rate)) +
  geom_polygon(color = "white", size = 0.1) +
  scale_fill_gradientn(
    colours = c("white", "#a6cee3", "#1f78b4", "#6a3d9a", "#4b0082"),
    na.value  = "grey90",
    labels    = percent_format(accuracy = 0.1),
    name      = "Avg Annual Change",
    guide     = guide_colorbar(
      title.position   = "top",
      label.position   = "bottom",
      barwidth         = unit(12, "cm"),   # make the bar long
      barheight        = unit(1, "cm"),    # make the bar thicker
      title.hjust      = 0.5
    )
  ) +
  labs(
    title = "Mean Year-to-Year Growth Rate of Cost per Capita\n(2017–2022)",
    x     = NULL,
    y     = NULL
  ) +
  theme_void() +
...
# TOC distribution for CCR>500
data_ccr_nmp <- data_ccr_ad %>%
  filter(!state_code=='MP')


ccr_outlier <- data_ccr_ad %>%
  filter(ccr_ad>=10)
